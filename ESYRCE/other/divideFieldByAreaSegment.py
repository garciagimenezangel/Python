...
Divide a field of a geodataframe by the area of the ESYRCE segment
"""
...
for file in files:
    ...
    # Some few segments may have larger area because the original data may come with equal D2_NUM for segments that are actually different. Rule out those segments because the segment metrics depending on area will be wrong.
    if len(indLargeArea)>0:
        print("Warning file "+file+": some segments ruled out because dissolved regions are not valid. Number ruled out:",len(indLargeArea)," out of: ",len(data))
